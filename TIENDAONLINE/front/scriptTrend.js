const getProducts = async() => {
    console.log("Estamos en tendencia");
    id = {
        id:'trend'
    };

    let respURL = await fetch(`http://localhost:3001/trend`);
    await respURL.json().then((s) => respURL = s);

    console.log(respURL);
    
    for (let i = 0; i < respURL.length; i++) {
        let productosTendencia = await searchProducts(JSON.stringify(respURL[i].keyword)); 
        
        console.log("producatos tendencia ", productosTendencia)
        CreateArticle(productosTendencia);
    }
